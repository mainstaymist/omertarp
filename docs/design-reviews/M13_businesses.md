...
Shared files legitimately use their dependencies at include time: an item catalogue calls `Omerta.Items.Register`, a venue list validates prices against `Omerta.Money`. So the fix is the loader, not this milestone.

**The first attempt at that fix was wrong in an instructive way.** It parsed each module's `depends` out of its source before including anything — which worked perfectly on the server and did nothing at all on the client, where gamemode Lua lives in a cache `file.Read` cannot see. Every directory came back unreadable, the whole set fell through to the alphabetical fallback, and the client failed exactly as before while the server was fine. A fix that works on one realm and silently no-ops on the other is worse than no fix, because it looks like it worked.

The mechanism that replaced it reads nothing. Every module directory now carries an **`sh_module.lua` containing only its `Register` call**; the loader includes all fifteen of those first, so the entire graph is known before any real code runs, then includes everything else in the order `resolveOrder` produces — the same topological sort the lifecycle already used, so include order and lifecycle order cannot drift apart. `depends` now means both.

Two lints hold the convention: every directory must have an `sh_module.lua` registering its own name, and that file must do nothing but register — anything it touched would run before the other modules had registered, which is the problem the phase split exists to remove. And a test **boots the real graph headlessly**, including every shared and server file in the resolved order, so a module reaching for something that has not loaded fails in the suite rather than in a server console.

The graph resolves to `database, accounts, seasons, characters, hud, interaction, identity, chat, inventory, organizations, treasury, business, demo, phone, population`.
