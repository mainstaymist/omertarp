...
**Staff-ergonomics addition (post-first-run):** the self-test consumes a season id (its `__selftest__` season is created then deleted, and auto-increment does not reuse), so ids drift and scraping them from logs is a poor operator experience for a command set that will be used for years. Added `omerta_season_list` (id, state, label, dates), and `omerta_season_start` now takes an **optional** id — with none, it starts the sole season in `setup`, refusing with the ambiguous ids named if there is more than one. Resolution logic is pure and unit-tested (`Internal.PickSoleSetupSeason`).

In-engine acceptance (user-side): pull, restart (watch for `applying migration 2: seasons and path selection`), then `omerta_seasons_selftest`. For full coverage: `omerta_season_create Season One`, `omerta_season_start` (no id needed), re-run the self-test — the path steps now execute — and `omerta_season_list` / `omerta_season_status` at will. Leave the season running; M4 needs it.
