...
- **Module lifecycle vs. inclusion**: `depends` orders lifecycle calls only. Include-time code defines, `OnEnable` connects — documented in `core/sh_module.lua`'s header.
- **File ordering within a module (revised during M4):** `sh_` files are included **before** `cl_`/`sv_` files, alphabetically within each realm. Originally the loader sorted purely alphabetically, which puts `cl_` before `sh_` — so on the client a UI file ran before the shared file defining its constants, and M4's creation UI crashed at include time. Shared files define the contracts realm files consume, so shared must load first. Covered by a regression check in `tests/test_module.lua`. Cross-*module* references still belong in `OnEnable` or later.
...
- **Project-wide hook convention (learned from the first in-engine boot):** the `GM` global is valid only while gamemode files are being included; the engine clears it afterwards, so any hook body referencing `GM` indexes nil at runtime. Inside hooks use `self` (or `GAMEMODE`). Every module from M1 onward defines hooks, so this is enforced automatically by a linter check in `tests/test_lint.lua` rather than left to review discipline.
